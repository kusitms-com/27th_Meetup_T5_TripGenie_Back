...
import com.simpletripbe.moduledomain.login.oauth.OauthService;
...
public class MainUserService {
    ...
    public CommonResponse signIn(String idToken) throws JsonProcessingException {
        ...
        Optional<User> userOptional = userRepository.findByUserId(socialOAuthDTO.getEmail());
        ...
        if (userOptional.isPresent()) {
            ...
            // 최초 로그인인 경우 -> 추가 정보를 입력받기 위해 jwt access token 생성을 위한 임시 회원가입 진행
    ...
    public CommonResponse signUp(SignUpReq signUpReq) {
        ...
        Optional<User> userOptional = userRepository.findByUserId(signUpReq.getEmail());

        // 유저가 존재하는 경우
        if (userOptional.isPresent()) {
            // 임시 회원가입 상태인 경우 회원가입 로직 진행
